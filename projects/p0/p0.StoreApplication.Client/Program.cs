...
using Serilog;
using dm = p0.StoreApplication.Domain.Models;
...
namespace p0.StoreApplication.Client
{
  ...
  public class Program
  {
    ...
    private static readonly OrderProductSingleton _orderProductSingleton = OrderProductSingleton.Instance;
    ...
    public bool ValidateData(int options, string value)
    {
      ...
      if (int.TryParse(value, out int option))
      {
        ...
        Log.Information("Invalid data: Out of bounds");
        return false;
      }
    ...
    {
      var data = _productSingleton.QueryProductList(store);
    ...
    private void SelectProduct(dm.Store store, out dm.Product product)
    {
      ...
      var products = (_productSingleton.QueryProductList(store));
    ...
    public string MakeOrder(dm.Customer cu, dm.Store st, List<dm.Product> ca)
    {
      ...
      else
      {
        ...
        _orderSingleton.Add(new dm.StoreOrder() { CustomerId = cu.CustomerId, StoreId = st.StoreId, OrderDate = DateTime.Now });
        //Sets the last order inserted
        dm.StoreOrder lastOrder = _orderSingleton.QueryLastOrder();
        ...
        foreach (var product in ca)
        {
          ...
          dm.OrderProduct orderProduct = _orderProductSingleton.GetOrderProduct(product.ProductId, lastOrder.OrderId);
          //If the record does not exist, add a new record with a quantity of 1.
          if (orderProduct == null)
            _orderProductSingleton.Add(new dm.OrderProduct() { OrderId = orderId, ProductId = productId, Quantity = 1 });
          //If the record exists in the database, increment the quantity
          else
          {
            short newQuantity = (short)(orderProduct.Quantity + 1);
            _orderProductSingleton.Update(productId, orderId, newQuantity);
          }
        }
    ...
    private static void OutputOrder<StoreOrder>(dm.Customer customer)
    {
      ...
      List<dm.StoreOrder> data = _orderSingleton.QueryOrders(customer);
      if (data.Count == 0)
      {
    ...
    private static void OutputOrder<StoreOrder>(dm.Store store)
    {
      ...
      List<dm.StoreOrder> data = _orderSingleton.QueryOrders(store);
      if (data.Count == 0)
      {
    ...
    private static void PrintOrders(List<dm.StoreOrder> data)
    {
      foreach (var item in data)
      {
        ...
        Console.WriteLine($"Customer: {_customerSingleton.GetCustomer(item.CustomerId).Name}");
        Console.WriteLine($"Store: {_storeSingleton.GetStore(item.StoreId)}");
        ...
        var productData = _productSingleton.QueryProductList(item);
        Console.Write("Products: ");
        foreach (var pItem in productData)
        {
          Console.WriteLine(pItem + " | Quantity: " + _orderProductSingleton.GetOrderProduct(pItem.ProductId,item.OrderId).Quantity);
        }
        Console.WriteLine("");
      }
    }
